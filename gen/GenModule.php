 ...
 * the following URL:
 *
 * http://localhost/path/to/index.php?r=gen
 *
 ...
 *         'rules'=>array(
 *             'gen'=>'gen',
 *             'gen/<controller:\w+>'=>'gen/<controller>',
 *             'gen/<controller:\w+>/<action:\w+>'=>'gen/<controller>/<action>',
 *             ...other rules...
 *         ),
 *     )
 * )
 ...
 *
 * http://localhost/path/to/index.php/gen
 *
 * @property string $assetsUrl The base URL that contains all published asset files of gen.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.gen
 * @since 1.1.2
 */
class GenModule extends CWebModule
{
	/**
	 ...
	 * If you want to allow all IPs to access gen, you may set this property to be false
	 * (DO NOT DO THIS UNLESS YOU KNOW THE CONSEQUENCE!!!)
	 ...
	 * Defaults to array('application.gen').
	 */
	public $generatorPaths=array('application.gen');
	/**
	...
	{
		parent::init();
		Yii::setPathOfAlias('gen',dirname(__FILE__));
		...
		$this->generatorPaths[]='gen.generators';
		$this->controllerMap=$this->findGenerators();
	}

	/**
	 * @return string the base URL that contains all published asset files of gen.
	 */
	public function getAssetsUrl()
	{
		if($this->_assetsUrl===null)
			$this->_assetsUrl=Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('gen.assets'));
		return $this->_assetsUrl;
	}

	/**
	 * @param string $value the base URL that contains all published asset files of gen.
	 */
	...
	protected function findGenerators()
	{
		...
		for($i=$n-1;$i>=0;--$i)
		{
			...
			{
				if($name[0]!=='.' && is_dir($path.'/'.$name) && $name != 'model')
				{
