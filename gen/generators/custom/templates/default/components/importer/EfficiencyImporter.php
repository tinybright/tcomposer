...
class HolderImporter extends BaseImporter {
    /**
    ...
    public function doImport()
    {
        for ($row = 2;$row <= $this->highestRow ;$row++){
            ...
            $addRet = EfficiencyService::addEfficiency($this->uid,$rawData);
            if ($addRet['ret']=='FAIL') {
    ...
    public static function import($uid,$files){
        $importer = new EfficiencyImporter($uid,$files);
        $importer->objName = TplUtil::getObjName('Efficiency');
