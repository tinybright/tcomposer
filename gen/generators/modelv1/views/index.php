...
Yii::app()->clientScript->registerScript('gen.model',"
$('#{$class}_connectionId').change(function(){
	var tableName=$('#{$class}_tableName');
	tableName.autocomplete('option', 'source', []);
	$.ajax({
		url: '".Yii::app()->getUrlManager()->createUrl('gen/modelv1/getTableNames')."',
		data: {db: this.value},
		dataType: 'json'
	}).done(function(data){
...
$('#{$class}_tableName').bind('keyup change', function(){
	...
	if(!model.data('changed')) {
		...
		var lineIndex = tableName.indexOf('_');
		if(lineIndex == 1){
...
<?php $form=$this->beginWidget('CCodeForm', array('model'=>$model)); ?>
	...
	<div class="row sticky">
		...
		<?php echo $form->dropDownList($model,'connectionId',[
			'db'=>'db',
			'dbmenu'=>'dbmenu'
		]); ?>
		<div class="tooltip">
	...
	<div class="row sticky">
		...
		<?php echo $form->textField($model,'tablePrefix', array('size'=>65,'disabled'=>'disabled')); ?>
		<div class="tooltip">
	...
	<div class="row">
		...
		<?
		/*$a = Yii::app()->hasComponent($model->connectionId) ? array_keys(Yii::app()->{@$model->connectionId}->schema->getTables()) : array();
		echo '<pre>';
		foreach ($a as $one){
			echo 'DROP TABLE '."`".$one."`;\n";
		}*/
